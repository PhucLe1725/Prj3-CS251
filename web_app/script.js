...
var contractAddress = "0x2279B7A0a67DB372996a5FaB50D91eAA73d2eBe6"; // FIXME: fill this in with your contract's address/hash
...
async function getEdge(node) {
	let edge = await BlockchainSplitwise.getEdge(node);
	return edge;
}
...
async function getUsers() {
	let listOfUser = await BlockchainSplitwise.getListofUsers();
	return listOfUser;
}

// TODO: Get the total amount owed by the user specified by 'user'
async function getTotalOwed(user) {
	return BlockchainSplitwise.getTotalOwed(user).catch(err => err.message);
}
...
async function getLastActive(user) {
	let lastActiveTime = await BlockchainSplitwise.getLastActive(user);
...
async function add_IOU(creditor, amount) {
	let sender = defaultAccount;
	let path = await doBFS(creditor, sender, getEdge);
	let check = path !== null;
	if (!check){
		path = [sender];
	}
	await BlockchainSplitwise.add_IOU(creditor, amount, path, check, sender);
}
